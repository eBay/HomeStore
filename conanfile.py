#!/usr/bin/env python
# -*- coding: utf-8 -*-
from conans import ConanFile, CMake, tools

class HomestoreConan(ConanFile):
    name = "homestore"

    version = "0.11.17"


    license = "Proprietary"
    url = "https://github.corp.ebay.com/SDS/Homestore"
    description = "HomeStore"

    settings = "arch", "os", "compiler", "build_type", "sanitize"
    options = {"shared": ['True', 'False'],
               "fPIC": ['True', 'False'],
               "coverage": ['True', 'False']}
    default_options = 'shared=False', 'fPIC=True', 'coverage=False'

    requires = (
                "benchmark/1.5.0@oss/stable",
                "boost_asio/1.69.0@bincrafters/stable",
                "boost_dynamic_bitset/1.69.0@bincrafters/stable",
                "boost_circular_buffer/1.69.0@bincrafters/stable",
                "boost_heap/1.69.0@bincrafters/stable",
                "boost_intrusive/1.69.0@bincrafters/stable",
                "boost_preprocessor/1.69.0@bincrafters/stable",
                "boost_uuid/1.69.0@bincrafters/stable",
                "double-conversion/3.1.4@bincrafters/stable",
                "farmhash/1.0.0@oss/stable",
                "folly/2019.07.22.00@bincrafters/testing",
                "gtest/1.8.1@bincrafters/stable",
                "iomgr/2.2.6@sds/testing",
                "libevent/2.1.10@bincrafters/stable",
                "lzma/5.2.4@bincrafters/stable",
                "sisl/0.3.6@sisl/develop",
                "sds_logging/5.2.0@sds/testing",
                "sds_options/0.1.5@sds/testing",
                "isa-l/2.21.0@oss/stable",
                "flip/0.1.1@sds/testing",
                "zstd/1.4.0@bincrafters/stable",
                )

    generators = "cmake"
    exports_sources = "cmake/*", "src/*", "CMakeLists.txt"

    def configure(self):
        if self.settings.sanitize != None:
            del self.options.coverage

    def requirements(self):
        if not self.settings.build_type == "Debug":
            self.requires("gperftools/2.7.0@oss/stable")

    def build(self):
        cmake = CMake(self)

        definitions = {'CONAN_BUILD_COVERAGE': 'OFF',
                       'CMAKE_EXPORT_COMPILE_COMMANDS': 'ON',
                       'MEMORY_SANITIZER_ON': 'OFF'}
        test_target = None

        if self.settings.sanitize != "address" and self.options.coverage == 'True':
            definitions['CONAN_BUILD_COVERAGE'] = 'ON'
            test_target = 'coverage'

        if self.settings.sanitize != None:
            definitions['MEMORY_SANITIZER_ON'] = 'ON'

        if self.settings.build_type == 'Debug':
            definitions['CMAKE_BUILD_TYPE'] = 'Debug'

        cmake.configure(defs=definitions)
        cmake.build()
        cmake.test(target=test_target, output_on_failure=True)

    def package(self):
        self.copy("*.h", dst="include", src="src", keep_path=True)
        self.copy("*.hpp", dst="include", src="src", keep_path=True)
        self.copy("*/btree_node.cpp", dst="include", src="src", keep_path=True)
        self.copy("*cache/cache.cpp", dst="include", src="src", keep_path=True)
        self.copy("*homeblks.so", dst="lib", keep_path=False)
        self.copy("*homeblks.dll", dst="lib", keep_path=False)
        self.copy("*homeblks.dylib", dst="lib", keep_path=False)
        self.copy("*homeblks.lib", dst="lib", keep_path=False)
        self.copy("*homeblks.a", dst="lib", keep_path=False)
        self.copy("*obj_allocator_benchmark", dst="bin")
        self.copy("*perf_cache", dst="bin")
        self.copy("*perf_test_volume", dst="bin")
        self.copy("*test_avector", dst="bin")
        self.copy("*test_bitset", dst="bin")
        self.copy("*test_bitword", dst="bin")
        self.copy("*test_blkalloc", dst="bin")
        self.copy("*test_btree_crud", dst="bin")
        self.copy("*test_cache", dst="bin")
        self.copy("*test_composite_allocator", dst="bin")
        self.copy("*test_hash", dst="bin")
        self.copy("*test_iomgr_exec", dst="bin")
        self.copy("*test_load", dst="bin")
        self.copy("*test_mapping", dst="bin")
        self.copy("*test_obj_allocator", dst="bin")
        self.copy("*test_threadpool", dst="bin")
        self.copy("*test_volume", dst="bin")
        self.copy("*vol_test.py", dst="bin")

    def package_info(self):
        self.cpp_info.libs = tools.collect_libs(self)
        self.cpp_info.cxxflags.append("-DBOOST_ALLOW_DEPRECATED_HEADERS")
        if self.settings.sanitize != None:
            self.cpp_info.sharedlinkflags.append("-fsanitize=address")
            self.cpp_info.exelinkflags.append("-fsanitize=address")
            self.cpp_info.sharedlinkflags.append("-fsanitize=undefined")
            self.cpp_info.exelinkflags.append("-fsanitize=undefined")
        elif self.options.coverage == 'True':
            self.cpp_info.libs.append('gcov')
        if self.settings.os == "Linux":
            self.cpp_info.libs.extend(["aio"])

    def deploy(self):
        self.copy("*obj_allocator_benchmark", dst="/usr/local/bin")
        self.copy("*perf_cache", dst="/usr/local/bin")
        self.copy("*perf_test_volume", dst="/usr/local/bin")
        self.copy("*test_avector", dst="/usr/local/bin")
        self.copy("*test_bitset", dst="/usr/local/bin")
        self.copy("*test_bitword", dst="/usr/local/bin")
        self.copy("*test_blkalloc", dst="/usr/local/bin")
        self.copy("*test_btree_crud", dst="/usr/local/bin")
        self.copy("*test_cache", dst="/usr/local/bin")
        self.copy("*test_composite_allocator", dst="/usr/local/bin")
        self.copy("*test_hash", dst="/usr/local/bin")
        self.copy("*test_iomgr_exec", dst="/usr/local/bin")
        self.copy("*test_load", dst="/usr/local/bin")
        self.copy("*test_mapping", dst="/usr/local/bin")
        self.copy("*test_obj_allocator", dst="/usr/local/bin")
        self.copy("*test_threadpool", dst="/usr/local/bin")
        self.copy("*test_volume", dst="/usr/local/bin")
        self.copy("*vol_test.py", dst="/usr/local/bin")
