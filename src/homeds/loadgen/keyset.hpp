...
struct key_info_ptr {
    ...
    key_info_ptr(const key_info_ptr& p){
        ...
        if(m_is_mutate) m_ki->m_mutate_count++;
        else m_ki->m_read_count++;
    }
...
public:
    ...
    int update_contigious_kv(std::vector< key_info_ptr< K, V > >& kv){
        ...
        while(keyit!=kv.end()){//keys and values
            ...
            if(pregenerated){
                ...
                std::shared_ptr<V>  temp = std::make_shared<V>(V::gen_value(ValuePattern::SEQUENTIAL_VAL,val.get()));

                if(!val->is_consecutive(*temp.get()))break;//rolled over values
                ...
                if(result.second==false){LOGERROR("generated value is not unique!"); assert(0);}
            }
    ...
    key_info_ptr< K, V > generate_and_put_key(KeyPattern gen_pattern) {
        ...
        if(result.second==false){LOGERROR("generated key is not unique!"); assert(0);}
        return kip;
    }
    ...
    std::vector< key_info_ptr< K, V > > get_consecutive_keys(KeyPattern pattern, bool exclusive_access, bool is_mutate,
                                                            uint32_t num_needed){
        std::unique_lock l(m_rwlock);
        std::vector< key_info_ptr< K, V > > kips = _get_contigoous_keys(pattern, exclusive_access, is_mutate, num_needed);
        if(kips.size()<=1)
            return kips;
    ...
    std::vector< key_info_ptr< K, V > > get_keys(KeyPattern pattern, uint32_t n, bool is_mutate, bool exclusive_access) {
    ...
    void _remove_value(std::shared_ptr<V> value){
        ...
        if(x==0){LOGERROR("Value not found!"); assert(0);}
    }
    ...
    void remove_key(key_info_ptr< K, V >& kip) {
        ...
        if(x==0){LOGERROR("Key not found!"); assert(0);}
        kip->mark_freed();
    ...
    key_info< K, V >* find_key(K& key) {
        ...
        delete ki;
        if(it==m_data_set.end())return nullptr;
    ...
    bool has_value(std::shared_ptr<V> value){
        ...
        if(it==m_value_set.end())return false;
        const std::shared_ptr<V> exp_value = *it;
        if(exp_value->compare(*value.get())!=0)return false;
        return true;
    }
...
private:
    ...
    std::vector< V > _get_contigoous_values(ValuePattern first_value_pattern, bool exclusive_access,
                                                             bool is_mutate, uint32_t num_needed) {
