...
#define INVALID_SEQ_ID UINT64_MAX
        ...
        public:
            ...
            virtual void verify(std::vector< key_info_ptr< K, V > > loadgenkv,
                 std::vector<std::pair<K, V>> storekv, store_error_cb_t error_cb, bool exclusive_access) override{
                ...
                while(l_itr!=loadgenkv.end() && s_itr!=storekv.end()){
                    ...
                    for (auto i = 0ul;i<CS_ARRAY_STACK_SIZE;i++) carr[i] = 1;
                ...
                if(l_itr==loadgenkv.end() && s_itr!=storekv.end()){
                    error_cb(generator_op_error::data_missing, nullptr, nullptr,
                             fmt::format("More data found than expected"));
            ...
            virtual bool range_update(K& start_key, bool start_incl, K& end_key, bool end_incl,
                ...
                ValueEntry ve(INVALID_SEQ_ID,BlkId(req->blkId.get_id(),req->blkId.get_nblks(),0),0,req->blkId.get_nblks(),carr);
