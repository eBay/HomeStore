...
struct LogEntry {
    ...
    LogEntry(uint64_t start_lba, uint64_t nblks, std::vector<crc_t>& crc) : m_start_lba(start_lba), m_nblks(nblks) {
        for (auto& c : crc) {
...
struct VolLogTracker {
    ...
    void append(uint64_t lba, uint64_t nblks, std::vector<crc_t>& crc) {
        ...
        if (m_cursor + 3 * sizeof(uint64_t) + nblks * sizeof(crc_t) > get_file_size()) {
            ...
            LOGINFO("WARNING: Log cursor will exceed max mapped file size, resetting cursor to 0. Old log buffer will be lost. ");
            m_cursor = 0;
        }
    ...
    void dump_history(uint64_t lba) {
        ...
        for (uint64_t cursor = 0; cursor < m_cursor; ) {
            ...
            if (lba >= start_lba && lba <= start_lba + nblks) {
                ...
                LOGINFO("vol_id: {},  write_id: {}, lba: {}, crc: {}", 
                        m_vol_id, write_id, lba,  
...
public: 
    ...
    void dump_history(uint64_t vol_id, uint64_t lba) {
        m_vols[vol_id]->dump_history(lba);
    }
