...
#ifndef LIBUTILS_homeds_BITSET_HPP_
#define LIBUTILS_homeds_BITSET_HPP_
...
public:
    static constexpr uint32_t size() { return (sizeof(T) * 8); }

    Bitword() { m_bits = 0; }

    explicit Bitword(T b) { m_bits = b; }
     ...
     */
    int get_reset_count() const { return size() - get_set_count(); }

    T set_bits(int start, int nbits) { return set_reset_bits(start, nbits, true /* set */); }

    T reset_bits(int start, int nbits) { return set_reset_bits(start, nbits, false /* set */); }
    ...
    bool set_next_reset_bit(int start, int* p_bit) { return set_next_reset_bit(start, size(), p_bit); }
...
public:
    ...
    int get_set_count() { return homeds::Bitword< T >(m_bits.load()).get_set_count(); }
    ...
    void set_reset_bits(int start, int nbits, bool set, std::memory_order order = std::memory_order_seq_cst) {
        ...
        do {
            ...
            newb = homeds::Bitword< T >(oldb).set_reset_bits(start, nbits, set);
    ...
    bool is_bit_set_reset(int start, bool check_for_set, std::memory_order order = std::memory_order_seq_cst) const {
        return homeds::Bitword< T >(m_bits.load(order)).is_bit_set_reset(start, check_for_set);
    }

    bool is_bits_set_reset(int start, int nbits, bool check_for_set,
                           std::memory_order order = std::memory_order_seq_cst) const {
        return homeds::Bitword< T >(m_bits.load(order)).is_bits_set_reset(start, nbits, check_for_set);
    }

    int get_next_reset_bit(int start, int* p_reset_start, std::memory_order order = std::memory_order_seq_cst) const {
        return homeds::Bitword< T >(m_bits.load(order)).get_next_reset_bit(start, p_reset_start);
    }
    ...
    void print() { homeds::Bitword< T >(m_bits.load()).print(); }
...
#endif /* LIBUTILS_homeds_BITSET_HPP_ */
