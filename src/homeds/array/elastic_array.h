...
namespace homeds {
    ...
    template<ArrayType>
    Elastic_Array<ArrayTypeParams>::Elastic_Array(int no_of_elements_capacity,
    ...
    template<ArrayType>
    Elastic_Array<ArrayTypeParams>::Elastic_Array(void *mem, uint32_t size,
                                                  uint32_t max_grow_to) {
    ...
    template<ArrayType>
    void
    Elastic_Array<ArrayTypeParams>::set_mem(void *mem, uint32_t size, uint32_t max_grow_to) {
    ...
    template<ArrayType>
    void
    Elastic_Array<ArrayTypeParams>::initialize(void *mem, uint32_t size, uint32_t max_grow_to) {
        m_size = size;
    ...
    template<ArrayType>
    void
    Elastic_Array<ArrayTypeParams>::free_mem_if_needed() {
        if (mallocated) {
            m_header = NULL;
            m_elements = NULL;
            free(m_mem);
    ...
    template<ArrayType>
    uint32_t
    Elastic_Array<ArrayTypeParams>::get_size() const {
        return m_size;
    }

    template<ArrayType>
    uint32_t
    Elastic_Array<ArrayTypeParams>::estimate_size_after_addOrUpdate(
    ...
    template<ArrayType>
    uint32_t
    Elastic_Array<ArrayTypeParams>::get_no_of_elements_filled() const {
        return m_header->m_no_of_elements_filled;
    }

    template<ArrayType>
    uint32_t
    Elastic_Array<ArrayTypeParams>::get_no_of_elements_total() const {
        return m_no_of_elements_total;
    }

    template<ArrayType>
    bool
    Elastic_Array<ArrayTypeParams>::addOrUpdate(ElementType *element) {
        std::lock_guard<std::recursive_mutex> lock(m_mutex);
    ...
    template<ArrayType>
    bool
    Elastic_Array<ArrayTypeParams>::removeIfPresent(ElementType *element) {
        std::lock_guard<std::recursive_mutex> lock(m_mutex);
        int pos = binary_search(element);
        if (pos >= 0) {
    ...
    template<ArrayType>
    ElementType *
    Elastic_Array<ArrayTypeParams>::operator[](uint32_t pos) {
        std::lock_guard<std::recursive_mutex> lock(m_mutex);
    ...
    template<ArrayType>
    std::string
    Elastic_Array<ArrayTypeParams>::to_string() const {
        uint32_t i = 0;
        std::stringstream ss;
    ...
    template<ArrayType>
    int
    Elastic_Array<ArrayTypeParams>::binary_search(ElementType *element) {
    ...
    template<ArrayType>
    bool
    Elastic_Array<ArrayTypeParams>::is_resize_required() {
    ...
    template<ArrayType>
    uint32_t
    Elastic_Array<ArrayTypeParams>::get_size_to_grow_to() {
    ...
    template<ArrayType>
    void
    Elastic_Array<ArrayTypeParams>::resize_array() {
    ...
    template<ArrayType>
    void
    Elastic_Array<ArrayTypeParams>::remove_shift(uint32_t spos, uint32_t epos) {
    ...
    template<ArrayType>
    void
    Elastic_Array<ArrayTypeParams>::insert_shift(uint32_t pos, ElementType *element) {
        if (pos > m_header->m_no_of_elements_filled || pos < 0) assert(0);
    ...
    template<ArrayType>
    void
    Elastic_Array<ArrayTypeParams>::update(uint32_t pos, ElementType *element) {
        memcpy((void *) (m_elements + pos), (void *) element, sizeOfElement);
