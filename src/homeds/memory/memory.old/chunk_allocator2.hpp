...
public:
	ChunkMemAllocator(uint32_t total_size) :
			m_base_ptr(nullptr),
...
private:
	...
	uint32_t hdr_to_id(chunk_pool_header *hdr) {
		return ( ( ((uint8_t *) hdr) - m_base_ptr) / ChunkSize );
	}

	chunk_pool_header *id_to_hdr(uint32_t id) {
		return ((chunk_pool_header *) (m_base_ptr + (ChunkSize * id)));
	}
