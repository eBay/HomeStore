...
public:
    TestLogStore(std::shared_ptr< HomeLogStore > store) : m_store_id{store->get_store_id()} { set_log_store(store); }

    void set_log_store(std::shared_ptr< HomeLogStore > store) {
        m_log_store = store;
        m_log_store->register_log_found_cb(bind_this(TestLogStore::on_log_found, 3));
    }
    ...
    void insert(logstore_seq_num_t start_lsn, int64_t parallel_count, bool wait_for_completion = true) {
        std::vector< logstore_seq_num_t > lsns;
        lsns.reserve(parallel_count);

        // Shuffle ids within the range for count
        for (auto lsn{start_lsn}; lsn < start_lsn + parallel_count; ++lsn) {
            lsns.push_back(lsn);
        }
        ...
        ASSERT_LT(m_log_store->get_contiguous_issued_seq_num(-1), start_lsn + parallel_count);
