...
public:
    void add_chunk(cshared< Chunk >& chunk) override {
        if (!m_fixed_chunk) { m_fixed_chunk = chunk; }
...
struct DeterministicChunkSelectorWbcReuseTest : public test_common::HSTestHelper,
    ...
    public:
        ...
        void warm_all_nodes_into_wbc() {
            ...
            for (auto const node_id : ids) {
                ...
                ASSERT_EQ(ret, btree_status_t::success) << "failed to read node " << blk_id{node_id}.to_string();
            }
        }
    ...
    void SetUp() override {
        ...
        this->start_homestore("test_index_crash_recovery",
    ...
    void destroy_btree(std::shared_ptr< InspectableBtree >& bt) {
        ...
        ASSERT_EQ(homestore::detail::sync_get(bt->destroy()), btree_status_t::success);
        bt.reset();
    }
    ...
    uint32_t assert_all_nodes_on_same_chunk(std::shared_ptr< InspectableBtree >& bt) {
        ...
        for (auto const id : ids) {
            ...
            DEBUG_ASSERT_EQ(chunk_num, expected_chunk, "node {} landed on chunk {} but expected chunk {}",
                            blk_id{id}.to_string(), chunk_num, expected_chunk);
        }
    ...
    void evict_wbc_and_clear_chunk_async(chunk_num_t chunk_num) {
        ...
        iomanager.run_on_forget(iomgr::reactor_regex::random_worker, [chunk, done]() {
...
TEST_F(DeterministicChunkSelectorWbcReuseTest, DestroyedBtreeStaleWbcEntriesCorruptNextBtreeOnSameChunk) {
    ...
    // If this is enabled, we get a SegFault on do_flush_one_buf()
    ...
    Chunk* chunk = hs()->device_mgr()->get_chunk_mutable(bt1_chunk);
    auto blk_alloc = chunk->blk_allocator();

    // Allocator-state validation: blocks should be used
    EXPECT_NE(blk_alloc->get_used_blks(), 0u);
    EXPECT_NE(blk_alloc->available_blks(), blk_alloc->get_total_blks());

    // Evict stale WBC entries for that chunk, then clear allocator state
    evict_wbc_and_clear_chunk_async(bt1_chunk);

    // Allocator-state validation: everything should look free again
    chunk = hs()->device_mgr()->get_chunk_mutable(bt1_chunk);
    blk_alloc = chunk->blk_allocator();
    EXPECT_EQ(blk_alloc->get_used_blks(), 0u);
    EXPECT_EQ(blk_alloc->available_blks(), blk_alloc->get_total_blks());
