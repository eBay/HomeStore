...
struct BtreeRecoveryTest : public BtreeTestHelper< TestType >, public ::testing::Test {
   ...
   public:
       ...
       std::shared_ptr< IndexTableBase > on_index_table_found(superblk< index_table_sb >&& sb) override {
           ...
           LOGINFO("super bnode_id {} version {}", sb->super_node, sb->super_link_version);
...
//   this->print_keys();
   LOGINFO("\n\n\n\n   \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t 2- enable crash");
   LOGINFO("\n\n\n\n\n \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t 2- Do Forward sequential insert [{}, {}) entries - size {}",part1, part2, this->m_bt->count_keys(this->m_bt->root_node_id()));
...
//   this->visualize_keys("tree2.dot");
   ...
   LOGINFO("\n\n\n\n\n \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t 3-Root bnode_id {} version {} ", this->m_bt->root_node_id(), this->m_bt->root_link_version());
...
//   this->visualize_keys("tree3.dot");
   ...
   LOGINFO("\n\n\n\n\n  \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t retry Do Forward sequential insert [{}, {}) entries",part1, part2);
   for (i=part1;i < part2; ++i) {
       LOGINFO("\n put retry {}", i);
