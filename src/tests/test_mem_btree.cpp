...
TYPED_TEST(BtreeTest, SimpleGC) {
    ...
    for (uint32_t i = 500; i <= 800; ++i) {
        this->move_to_tombstone(i, btree_status_t::success);
    }
    this->print_keys(" After tombstone [500,800] ");
    LOGINFO("Step 2: Do GC on the tree for keys in range [700, 900]");
    auto out = this->remove_tombstone(700, 900);
    ...
    this->print_keys(fmt::format(" After GC {} entries are still in range [700,900] ", out.size()));
    LOGDEBUG("GC {} keys:\n{}", out.size(), format_tombstoned(out));
}
