...
#define verify_jd(...)                                                                                                 \
    do {                                                                                                               \
        LOGINFO("{}", log_dev_jd->to_string());                                                                        \
        test.verify_journal_descriptor(log_dev_jd, {__VA_ARGS__});                                                     \
    } while (0)

    // Initially no chunks used and offsets are zero.
    uint64_t writesz = 0;
    verify_jd(.ds = 0x0, .end = 0, .writesz = writesz, .rsvdsz = 0, .chunks = 0, .trunc = false, .total = 0,
              .seek = 0x0);
    ...
    writesz = 2 * data_size;
    verify_jd(.ds = 0x0, .end = chunk_size, .writesz = writesz, .rsvdsz = 0, .chunks = 1, .trunc = false,
              .total = chunk_size, .seek = 0x0);
    ...
    verify_jd(.ds = 0x0, .end = 3 * chunk_size, .writesz = writesz, .rsvdsz = 0, .chunks = 3, .trunc = false,
              .total = 3 * chunk_size, .seek = 0x0);
    test.read_all();
    ...
    verify_jd(.ds = 0x0, .end = 4 * chunk_size, .writesz = writesz, .rsvdsz = 0, .chunks = 4, .trunc = false,
              .total = 4 * chunk_size, .seek = 0x0);
    ...
    verify_jd(.ds = 0x600000, .end = 4 * chunk_size, .writesz = writesz - trunc_sz, .rsvdsz = 0, .chunks = 4,
              .trunc = true, .total = 4 * chunk_size, .seek = 0x0);
    ...
    test.truncate(trunc_offset);
    verify_jd(.ds = 0xb00000, .end = 4 * chunk_size, .writesz = writesz - trunc_sz, .rsvdsz = 0, .chunks = 3,
              .trunc = true, .total = 3 * chunk_size, .seek = 0x0);

    // Restart homestore and restore the offsets.
    LOGINFO("Restart homestore");
    restart_restore();
    verify_jd(.ds = 0xb00000, .end = 4 * chunk_size, .writesz = writesz - trunc_sz, .rsvdsz = 0, .chunks = 3,
              .trunc = false, .total = 3 * chunk_size, .seek = 0x0);
    test.read_all();

    // Truncate all entries. Num chunks, total and write sz should be 0.
    LOGINFO("Truncating all entries");
    trunc_offset = log_dev_jd->tail_offset();
    test.truncate(trunc_offset);
    verify_jd(.ds = 0x2000000, .end = 0x2000000, .writesz = 0, .rsvdsz = 0, .chunks = 0, .trunc = true, .total = 0,
              .seek = 0x0);
