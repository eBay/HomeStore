...
VolInterface *homestore::vol_homestore_init(init_params &cfg) {
    return(HomeBlks::init(cfg));
}

VolInterface *
HomeBlks::init(init_params &cfg) {
...
HomeBlks::createVolume(vol_params &params) {
    ...
    try {
        ...
        auto new_vol = new Volume(params);
        it->second = new_vol->get_shared_ptr();
...
std::error_condition 
HomeBlks::removeVolume(boost::uuids::uuid const &uuid) {
...
std::shared_ptr<Volume>
HomeBlks::lookupVolume(boost::uuids::uuid const &uuid) {
...
void 
HomeBlks::vol_sb_init(vol_sb *sb) {
    /* allocate block */
...
HomeBlks::vol_sb_remove(vol_sb *sb) {
    /* TODO need to implement it when vol delete comes in */
}
...
void
HomeBlks::config_super_block_write(bool lock) {
...
void
HomeBlks::vol_sb_write(vol_sb *sb) {
    vol_sb_write(sb, false);
}

void 
HomeBlks::vol_sb_write(vol_sb *sb, bool lock) {
    homeds::MemVector mvec;
...
void
HomeBlks::add_devices() {
    m_dev_mgr->add_devices(m_cfg.devices, m_cfg.disk_init);
}

void
HomeBlks::vol_mounted(std::shared_ptr<Volume> vol, vol_state state) {
   m_cfg.vol_mounted_cb(vol, state);
   LOGINFO("vol mounted name {} state {}", vol->get_sb()->vol_name, state);
}

void
HomeBlks::vol_state_change(std::shared_ptr<Volume> vol, vol_state old_state, vol_state new_state) {
   m_cfg.vol_state_change_cb(vol, old_state, new_state);
}

homestore::BlkStore<homestore::VdevVarSizeBlkAllocatorPolicy> *
HomeBlks::get_data_blkstore() {
    return m_data_blk_store;
}

homestore::BlkStore<homestore::VdevFixedBlkAllocatorPolicy, BLKSTORE_BUFFER_TYPE> *
HomeBlks::get_metadata_blkstore() {
    return m_metadata_blk_store;
}

boost::intrusive_ptr<BlkBuffer>
HomeBlks::get_valid_buf(std::vector<boost::intrusive_ptr<BlkBuffer>> bbuf, bool &rewrite) {
...
HomeBlks::scan_volumes() {
    ...
    try {
        while (blkid.to_integer() != BlkId::invalid_internal_id()) {
            std::vector<boost::intrusive_ptr<BlkBuffer>> bbuf = m_sb_blk_store->read_nmirror(blkid, 
                    m_cfg.devices.size() - 1);
...
void
HomeBlks::create_data_blkstore(vdev_info_block *vb) {
    if (vb == nullptr) {
...
void
HomeBlks::create_metadata_blkstore(vdev_info_block *vb) {
    if (vb == nullptr) {
        struct blkstore_blob blob;
...
void
HomeBlks::create_sb_blkstore(vdev_info_block *vb) {
    if (vb == nullptr) {
        ...
        if (blob->blkid.to_integer() == BlkId::invalid_internal_id()) {
            ...
            throw homestore::homestore_exception("init was failed last time. Should retry it with init", homestore_error::init_failed); 
        }

        /* read and build the config super block */
        std::vector<boost::intrusive_ptr<BlkBuffer>> bbuf = m_sb_blk_store->read_nmirror(blob->blkid, 
                m_cfg.devices.size() - 1);
        bool rewrite = false;
        boost::intrusive_ptr<BlkBuffer> valid_buf = get_valid_buf(bbuf, rewrite); 
...
bool HomeBlks::is_ready() {
    return (m_rdy);
}

void 
HomeBlks::init_thread() {
...
void
HomeBlks::vol_scan_cmpltd(std::shared_ptr<Volume> vol, vol_state state) {
