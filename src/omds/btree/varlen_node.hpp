...
class VarValueNode : public AbstractNode {
    VarValueNode(BtreeConfig &cfg, bnodeid_t id, bool init_perp, bool init_trans) :
            AbstractNode(id, init_perp, init_trans) {
        this->set_node_type(BTREE_NODETYPE_VAR_VALUE);
        get_node_header()->m_last_offset = get_node_space() + cfg.get_node_size() - (uint8_t *) this;
    }
