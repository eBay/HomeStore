...
class blk_id : public omds::HashKey
{
public:
	blk_id(int id)
	{
...
class blk_entry : public omds::HashValue
{
public:
	...
	blk_entry(int id, char *contents) : m_blk_id(id)
	{
		strcpy(m_blk_contents, contents);
	}
	...
	virtual const HashKey *extract_key() const
	{
		return (const HashKey *)&m_blk_id;
	}
...
void insert_thread(omds::HashMap<omds::fixed_type_hash_set> *map, int start, int count)
{
...
int main(int argc, char** argv)
{
...

	{
		...
		cds::threading::Manager::attachThread();
		omds::HashMap<omds::fixed_type_hash_set> map(8000, 10);
