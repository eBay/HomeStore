...
class blk_entry : public omds::HashNode
{
public:
...
private:
    blk_id m_blk_id;
    omds::atomic_counter< uint32_t > m_ref;
    char m_blk_contents[32];
};

omds::IntrusiveHashSet<blk_id, blk_entry> *glob_set;
blk_entry **glob_entries;
const blk_id **glob_ids;

void setup(int count) {
    glob_set = new omds::IntrusiveHashSet<blk_id, blk_entry>(count/ENTRIES_PER_BUCKET);
