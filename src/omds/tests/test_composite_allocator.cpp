...
int main(int argc, char *argv[]) {
    ...
    //omds::ChunkMemAllocator< 2048, 102400 > allocator1;
    omds::SysMemAllocator allocator1;
    uint8_t *meta_blk;
    ...
    for (auto i = 0; i < TOTAL_ALLOCS; i++) {
        ...
        alloc_time_ns += omds::get_elapsed_time_ns(t1);

        //printf("Allocated ptr = %p, meta_blk = %p\n", ptr, meta_blk);
        Clock::time_point t2 = Clock::now();
        free(ptr);
        dealloc_time_ns += omds::get_elapsed_time_ns(t2);
    }
