...
public:
    ...
    virtual size_t get_hash_code() const
    {
        return m_hash_code;
    }
...
private:
    struct __ref_valid_t
    {
        ...
        __ref_valid_t(int64_t n)
        {
            memcpy(this, &n, sizeof(int64_t));
        }
...
struct hash_code
{
    size_t operator()(const LFHashValue& hv) const
    {
        return hv.extract_key()->get_hash_code();
    }

    size_t operator()(const LFHashKey& hk) const
    {
        return hk.get_hash_code();
    }
};
...
typedef cds::intrusive::MichaelList< cds::gc::DHP, LFHashValue,
    typename cds::intrusive::michael_list::make_traits<
        // hook option
        cds::intrusive::opt::hook< cds::intrusive::michael_list::base_hook< cds::opt::gc< cds::gc::DHP > > >
        // item comparator option
...
<
		...
		typename cds::intrusive::michael_set::make_traits<
			cds::opt::hash< hash_code >
    >::type
> fixed_type_hash_set;

//////////// Dynamic Size but Unique hash implementations //////////////////
typedef cds::intrusive::FeldmanHashSet
<
		cds::gc::DHP,
		LFHashValue,
...
{
public:
    LFHashMap(size_t maxcount, size_t nload) :
            m_set(maxcount, nload)
    {
    }
