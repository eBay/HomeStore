        ...
        if (child_node->is_merge_needed(m_bt_cfg) || is_repair_needed(child_node, child_info)) {
            ...
            if (node_end_idx > curr_idx) {
                ...
                if (ret == btree_status_t::merge_not_required) {
                    BT_NODE_LOG(DEBUG, my_node, "merge not required for child = {} keys: {}", curr_idx,
...
btree_status_t Btree< K, V >::merge_nodes(const BtreeNodePtr& parent_node, const BtreeNodePtr& leftmost_node,
    ...
    // if total to be deleted in merged caused parent node to be empty (don't merge) because parent key is going to be
    // empty (violation). If we delete the parent (intermediate node) then it can lead to domino effects which we don't
    // want to deal with
    if (new_nodes.size() == 0 && parent_node->total_entries() == old_nodes.size() + 1) {
        BT_NODE_LOG(DEBUG, parent_node,
                    " MERGE: QUIT : please leave at lost the first key for parent if all children are gonna die soon");
        ret = btree_status_t::merge_not_required;
        goto out;
    }
