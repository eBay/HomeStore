...
public:
    IndexTable(uuid_t uuid, uuid_t parent_uuid, uint32_t user_sb_size, const BtreeConfig& cfg,
        ...
        m_sb->m_uuid = uuid;
        m_sb->m_parent_uuid = parent_uuid;
    ...
    btree_status_t init(void *op_context) {
        auto ret = Btree< K, V >::init(op_context);
        m_sb->root_node = Btree< K, V >::root_node_id();
        return ret;
    }

    uuid_t uuid() const override { return m_sb->m_uuid; }
    ...
    btree_status_t query(BtreeQueryRequest< K >& query_req, std::vector< std::pair< K, V > >& out_values) {
        auto cpg = hs()->cp_mgr().cp_guard();
        query_req.m_op_context = (void*)cpg->context(cp_consumer_t::INDEX_SVC);
        return Btree< K, V >::query(query_req, out_values);
    }

    template < typename ReqT >
    btree_status_t get(ReqT& get_req) {
        auto cpg = hs()->cp_mgr().cp_guard();
        get_req.m_op_context = (void*)cpg->context(cp_consumer_t::INDEX_SVC);
        return Btree< K, V >::get(get_req);
    }
...
protected:
    ...
    void realloc_node(const BtreeNodePtr& node) {
...
//        }
    ...
    btree_status_t prepare_node_txn(const BtreeNodePtr& parent_node, const BtreeNodePtr& child_node,
        ...
        if ((child_idx_node->m_last_mod_cp_id > cp_ctx->id()) ||
