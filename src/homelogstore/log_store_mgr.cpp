...
void HomeLogStoreMgr::start(const bool format) {
    ...
    m_status_obj = m_hb->status_mgr()->create_object(
        "LogStore", "module", std::bind(&HomeLogStoreMgr::get_status, this, std::placeholders::_1));
    m_hb->status_mgr()->register_module(m_status_obj);
