...
read_again:
    ...
    if (header->magic_word() != LOG_GROUP_HDR_MAGIC) {
        LOGINFO("Logdev data not seeing magic at pos {}, must have come to end of logdev", m_cur_group_cursor);
        return ret_buf;
    }

    if (header->total_size() > m_cur_log_buf.size()) {
        LOGINFO("Logstream group size {} is more than available buffer size {}, reading from store",
    ...
    LOGTRACE("Logstream read log group of size={} nrecords={} from device offset={}", header->total_size(),
...
sisl::byte_view log_stream_reader::read_next_bytes(uint64_t nbytes) {
    ...
    LOGTRACE("LogStream read {} bytes from offset {} ", actual_read, prev_pos);
    if (actual_read != 0) {
    ...
    LOGINFO("LogStream not read enough bytes from offset {}, must be end of device, wrap around and try reading",
            prev_pos);
    store->lseek(0);
    actual_read = store->read((void*)buf.bytes(), nbytes);
    LOGINFO("LogStream read {} bytes from offset 0 ", actual_read);
    buf.set_size(actual_read);
    return buf;
}
} // namespace homestore