...
private:
    ...
    public:
        ThrdTask(TaskFunc&& func) : m_func{std::move(func)} {}
...
public:
    explicit ThreadPool(const std::uint32_t num_threads) : m_done{false}, m_work_queue{}, m_threads{} {
...
inline auto submit_job(TaskFunc&& func, Args&&... args) {
    ...
    return get_thread_pool(MAX_NUM_CONCURRENT_THREADS)
        .submit(std::forward< TaskFunc >(func), std::forward< Args >(args)...);
}
} // namespace homestore
