...
uint32_t Crc32::get(const SEBuf& buf, uint32_t seed) {
    return get(buf.buf, buf.len, seed);
}

uint32_t Crc32::get(const std::string& str, uint32_t seed) {
    return get(str.data(), str.size(), seed);
}
