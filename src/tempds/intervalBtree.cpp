...
template < typename blobT >
void intervalBtree< blobT >::modify_size(int num) {
...
template < typename blobT >
void intervalBtree< blobT >::insert_freeq(int i) {
...
template < typename blobT >
struct node* intervalBtree< blobT >::get_free_node() {
    if (free_root == NULL) {
    ...
    free_node->left_indx = free_node->right_indx = get_indx(free_node);

    return free_node;
}

template < typename blobT >
int intervalBtree< blobT >::merge(struct node& new_node, struct node& node) {
...
template < typename blobT >
int intervalBtree< blobT >::get_indx(node* new_node) {
    return ((new_node - node_mem) / sizeof(struct node));
}

/* TODO:rishabh: we should change it into RB tree */
template < typename blobT >
int intervalBtree< blobT >::insert(node* new_node, node* root) {
    if (alloc_root == NULL) {
        assert(root == NULL);
...
template < typename blobT >
int intervalBtree< blobT >::insert_blob(const blobT blob) {
...
template < typename blobT >
int intervalBtree< blobT >::compare(blobT blob, struct node* root) {
...
template < typename blobT >
uint32_t intervalBtree< blobT >::read_internal(uint64_t hi, uint64_t lo, struct node* root,
    ...
    if (ret == 0) {
        ...
        if (lo >= root->blob.lo() && hi <= root->blob.hi()) {
            ...
            temp_blob.modify_hi(hi);
            total_blks =
                read(temp_blob.lo() - 1, lo, mem[root->left_indx], &blobList) + temp_blob.hi() - temp_blob.lo() + 1;
            ...
            blobList.push_back(temp_blob);
            total_blks =
                read(hi, temp_blob.hi() + 1, mem[root->right_indx], &blobList) + temp_blob.hi() - temp_blob.lo() + 1;
            /* return total number of blocks */
            return (total_blks);
        } else if (lo < root->blob.lo() && hi > root->blob.hi()) {
            {
            ...
            }
            {
                ...
                temp_blob.modify_lo(lo);
                blobList.push_back(temp_blob);
                total_blks += read(hi, temp_blob.hi() + 1, mem[root->right_indx], &blobList) + temp_blob.hi() -
                    temp_blob.lo() + 1;
            }
            return total_blks;
...
template < typename blobT >
blobT intervalBtree< blobT >::get_root_blob(void) const {
    assert(alloc_root != NULL);
    return alloc_root->blob;
}

template < typename blobT >
void* intervalBtree< blobT >::modify(blobT blob, void* cookie) {
    /* TODO:rishabh: will do it later */
}

template < typename blobT >
void* intervalBtree< blobT >::del(blobT blob, void* cookie) {
    /* TODO:rishabh: will do it later */
}

template < typename blobT >
void* intervalBtree< blobT >::get_mem(void) const {
    return mem;
}

template < typename blobT >
uint64_t intervalBtree< blobT >::get_size(void) const {
    return size;
}

template < typename blobT >
void intervalBtree< blobT >::set_mem(void* omem, uint64_t size) {
