...
public:
    ...
    void process_metadata_completions(const volume_req_ptr& req) {
        ...
        while (st < req->lba + req->nlbas) {
            ...
            while (bst <= ben) {
                if (st > et)
                    assert(0); // more blks freeed than expected
                if (m_blk_id_arr[st] != bst)
                    assert(0); // blks mistmach
                bst++;
                st++;
            }
    ...
    void process_ev_common(int fd, void* cookie, int event) {
        ...
        if (issued_ios.load() == max_issue_ios)
            return;
        outstanding_ios++;
        issued_ios++;
    ...
    start:
        if (retry == MAX_BLK)
            assert(0); // cant allocated blk anymore
