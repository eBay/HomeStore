...
class mapping {
  ...
  public:
    ...
    std::error_condition get(uint64_t start_lba, uint32_t nblks,
        ...
        while (curr_lba <= end_lba) {
            ...
            while (i < (int)valueOffsetToBlkIdLst.size()) {
                ...
                uint64_t curr_range_end_lba = curr_range_start_lba + valueOffsetToBlkIdLst[i]->m_interval_length - 1;
                ...
                // add the range found
                mappingList.push_back(
                    std::make_shared< Lba_Block >(*(valueOffsetToBlkIdLst[i].get()), curr_range_start_lba, true));
        ...
        if (!atleast_one_lba_found) {
            mappingList.empty();
            error = homestore::make_error_condition(homestore_error::lba_not_exist);
        } else if (atleast_one_lba_not_found) {
            error = homestore::make_error_condition(homestore_error::partial_lba_not_exist);
        }
        return error;
    }
    ...
    std::error_condition put(boost::intrusive_ptr< volume_req > req, uint64_t lba_uint, uint32_t nblks,
                             struct BlkId blkid) {
        ...
        while (total_blocks != 0) {
            ...
            uint64_t no_of_offset =
                (uint64_t)total_blocks > (end_lba_for_range - lba + 1) ? (end_lba_for_range - lba + 1) : total_blocks;
            ...
            std::shared_ptr< BtreeValue > existing_val =
                static_pointer_cast< BtreeValue >(std::make_shared< MappingValue >());
            m_bt->put(key, value, homeds::btree::APPEND_IF_EXISTS_ELSE_INSERT,
            ...
            for (std::shared_ptr< MappingInterval > offBlkId : offsetToBlkIdLst) {
                std::shared_ptr< Free_Blk_Entry > fbe = std::make_shared< Free_Blk_Entry >(
                    offBlkId->m_value.m_blkid, (uint16_t)offBlkId->m_value.m_blkid_offset,
