...
#define HS_SUBMOD_LOG(level, mod, req, submod_name, submod_val, msg, ...)                                              \
    {                                                                                                                  \
        BOOST_PP_IF(BOOST_PP_IS_EMPTY(req), homestore::_dummy_req* r = nullptr, const auto& r = req);                  \
        LOG##level##MOD_FMT(BOOST_PP_IF(BOOST_PP_IS_EMPTY(mod), base, mod),                                            \
...
#define HS_VERBOSE_LOG(level, mod, req, submod_name, submod_val, addln_info, msg, ...)                                 \
    {                                                                                                                  \
        BOOST_PP_IF(BOOST_PP_IS_EMPTY(req), homestore::_dummy_req* r = nullptr, const auto& r = req);                  \
        LOG##level##MOD_FMT(BOOST_PP_IF(BOOST_PP_IS_EMPTY(mod), base, mod),                                            \
                            ([&](fmt::memory_buffer& buf, const char* m, auto&&... args) {                             \
...
#define HS_REQ_ASSERT(assert_type, cond, req, msg, ...)                                                                \
    assert_type##_ASSERT_FMT(                                                                                          \
        cond,                                                                                                          \
        [&](fmt::memory_buffer& buf, const char* m, auto&&... args) {                                                  \
            homestore::hs_assert_formatter(buf, m, BOOST_PP_IF(BOOST_PP_IS_EMPTY(req), "", req->to_string()),          \
                                           args...);                                                                   \
        },                                                                                                             \
        msg, ##__VA_ARGS__)
#define HS_ASSERT(assert_type, cond, msg, ...) HS_REQ_ASSERT(assert_type, cond, , msg, ##__VA_ARGS__)
#define HS_SUBMOD_ASSERT(assert_type, cond, req, submod_name, submod_val, msg, ...)                                    \
    assert_type##_ASSERT_FMT(                                                                                          \
        cond,                                                                                                          \
        [&](fmt::memory_buffer& buf, const char* m, auto&&... args) {                                                  \
            homestore::hs_submod_assert_formatter(buf, m, BOOST_PP_IF(BOOST_PP_IS_EMPTY(req), "", req->to_string()),   \
...
#define HS_REQ_ASSERT_CMP(assert_type, val1, cmp, val2, req, ...)                                                      \
    assert_type##_ASSERT_CMP(                                                                                          \
        val1, cmp, val2,                                                                                               \
        [&](fmt::memory_buffer& buf, const char* m, auto&&... args) {                                                  \
            homestore::hs_cmp_assert_formatter(buf, m, BOOST_PP_IF(BOOST_PP_IS_EMPTY(req), "", req->to_string()),      \
                                               args...);                                                               \
        },                                                                                                             \
        ##__VA_ARGS__)
#define HS_ASSERT_CMP(assert_type, val1, cmp, val2, ...)                                                               \
...
#define HS_SUBMOD_ASSERT_CMP(assert_type, val1, cmp, val2, req, submod_name, submod_val, ...)                          \
    assert_type##_ASSERT_CMP(                                                                                          \
        val1, cmp, val2,                                                                                               \
        [&](fmt::memory_buffer& buf, const char* m, auto&&... args) {                                                  \
            homestore::hs_submod_cmp_assert_formatter(                                                                 \
                buf, m, BOOST_PP_IF(BOOST_PP_IS_EMPTY(req), "", req->to_string()), submod_name, submod_val, args...);  \
        },                                                                                                             \
        ##__VA_ARGS__)
...
namespace homestore {
            ...
            template < typename... Args >
            void hs_assert_formatter(fmt::memory_buffer & buf, const char* msg, const std::string& req_str,
                                     const Args&... args) {
                if (req_str.size()) {
                    fmt::format_to(buf, "\n[request={}]", req_str);
                }
            ...
            void hs_submod_assert_formatter(fmt::memory_buffer & buf, const char* msg, const std::string& req_str,
                                            const char* submod_name, const std::string& submod_val,
                                            const Args&... args) {
                fmt::format_to(buf, "\n[{}={}]", submod_name, submod_val);
