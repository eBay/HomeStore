...
void Blk_Read_Tracker::insert(const Free_Blk_Entry& fbe) {
    const uint64_t hash_code{static_cast< uint64_t >(std::hash< BlkId >()(fbe.get_base_blkid()))};
    BlkEvictionRecord* ber = BlkEvictionRecord::make_object(fbe.get_base_blkid());
...
void Blk_Read_Tracker::remove(const Free_Blk_Entry& fbe) {
    const uint64_t hash_code{static_cast< uint64_t >(std::hash< BlkId >()(fbe.get_base_blkid()))};
...
void Blk_Read_Tracker::safe_free_blks(const Free_Blk_Entry& fbe) {
    BlkId bid{fbe.get_base_blkid()};
