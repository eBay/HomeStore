...
void HomeStoreCPMgr::write_hs_cp_sb(hs_cp* hcp) {
...
void HomeStoreCPMgr::cp_reset(hs_cp* hcp) {
    m_wd_cp.cp_reset();
    auto cp = cp_io_enter();
    if (!cp) return;
...
void IndxMgr::recover_meta_ops() {
    ...
    for (uint32_t i = 0; i < meta_blk_list.size(); ++i) {
        ...
        case INDX_UNMAP: {
            sisl::byte_view buf = meta_blk_list[i].second;
            ...
            auto unmap_meta_blk_cntx = meta_blk_list[i].first;

            this->do_remaining_unmap_internal(nullptr, unmap_meta_blk_cntx, key, unmap_hdr->seq_id, btree_cur);
            break;
        }
        case SNAP_DESTROY:
            HS_ASSERT(DEBUG, 0, "invalid op");
...
void IndxMgr::flush_alloc_blks(const indx_cp_ptr& icp, hs_cp* hcp) {
    ...
    if (icp->flags & indx_cp_state::diff_cp) { icp->dcp.diff_tbl->flush_alloc_blks(icp->dcp.bcp, hcp->ba_cp); }
}
...
indx_cp_ptr IndxMgr::attach_prepare_indx_cp(const indx_cp_ptr& cur_icp, hs_cp* cur_hcp, hs_cp* new_hcp) {
    ...
    auto is_ba_cp = cur_icp->flags & indx_cp_state::ba_cp;
...
indx_cp_ptr IndxMgr::create_new_indx_cp(const indx_cp_ptr& cur_icp) {
    ...
    const seq_id_t dcp_start_seq_id =
        cur_icp->flags & indx_cp_state::diff_cp ? cur_icp->dcp.end_seqid : cur_icp->dcp.start_seqid;
