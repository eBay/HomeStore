...
std::shared_ptr< BlkAllocator > VirtualDev::create_blk_allocator(const blk_allocator_type_t btype) {
    switch (btype) {
...
void VirtualDev::init(DeviceManager* const mgr, vdev_info_block* const vb, vdev_comp_cb_t cb, const uint32_t page_size,
 ...
 */
virtual void add_chunk(PhysicalDevChunk* const chunk) {
...
ssize_t pwritev(const iovec* const iov, const int iovcnt, const off_t offset,
    ...
    HS_ASSERT_CMP(RELEASE, len, <=, m_reserved_sz, "Write size:{} larger then reserved size: {} is not allowed!", len,
                  m_reserved_sz);
...
off_t process_pwrite_offset(const size_t len, const off_t offset, uint32_t& dev_id, uint32_t& chunk_id,
    ...
    HS_ASSERT_CMP(RELEASE, m_chunk_size - offset_in_chunk, >=, len, "Writing size: {} crossing chunk is not allowed!",
                  len);
