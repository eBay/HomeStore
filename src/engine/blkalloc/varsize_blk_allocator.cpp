...
void VarsizeBlkAllocator::sweeper_thread(const size_t thread_num) {
    ...
    while (!s_sweeper_threads_stop) {
        ...
        {
            std::unique_lock< std::mutex > lock{s_sweeper_mutex};
            const auto woken{s_sweeper_cv.wait_for(
...
bool VarsizeBlkAllocator::prepare_sweep(BlkAllocSegment* const seg, const bool fill_entire_cache) {
    m_sweep_segment = seg;
