...
public:
    FixedBlkAllocatorTest() : m_alloced_count(0), m_total_space(1 * 1024 * 1024), m_blk_size(8 * 1024) {
...
public:
    VarsizeBlkAllocatorTest() : m_alloced_count(0), m_total_space(1 * 1024 * 1024), m_blk_size(512) {
    ...
    static void alloc_var_blocks(VarsizeBlkAllocatorTest* test, uint8_t nblks, std::vector< BlkId >* out_blist) {
        while (true) {
            BlkId bid;
            blk_alloc_hints hints;
            hints.desired_temp = 0;
