...
public:
    BtreePrefixNode(bnodeid_t id, bool initPerp, bool initTrans) : BtreeAbstractNode(id, initPerp, initTrans) {
    ...
    inline uint8_t* getRecordAreaStart() { return getNodeSpace() + sizeof(BtreePrefixHdr); }

    inline uint32_t getPerRecordSize() { return sizeof(BtreePrefixRecord); }

    inline BtreePrefixRecord* getRecordPtr(uint32_t ind) {
        return (BtreePrefixRecord*)(getRecordAreaStart() + (getPerRecordSize() * ind));
    }

    inline uint32_t getTotalRecordSize() { return getTotalEntries() * getPerRecordSize(); }
...
public:
    void setDataSize(uint16_t sz) { m_dataSize = sz; }
    void setDataOffset(uint16_t o) { m_dataOffset = o; };
    void setPrefixRecordIndex(uint16_t ind) { m_prefixRecordInd = ind; }
    void setPrefixSize(uint16_t sz) { m_prefixDataSize = sz; };

    uint16_t getDataSize() { return m_dataSize; }
    uint16_t getDataOffset() { return m_dataOffset; }
    uint16_t getPrefixRecordIndex() { return m_prefixRecordInd; }
    uint16_t getPrefixSize() { return m_prefixDataSize; }
