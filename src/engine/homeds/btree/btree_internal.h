...
struct btree_journal_entry {
    static constexpr size_t alloc_increment = 512;

    static btree_journal_entry* make(journal_op op) {
        size_t size = std::max(alloc_increment, sizeof(btree_journal_entry));
        return new (malloc(size)) btree_journal_entry(op);
    }

    static btree_journal_entry* realloc_if_needed(btree_journal_entry** ppentry, uint16_t append_size) {
        btree_journal_entry* entry = *ppentry;
        uint16_t avail_size = alloc_increment - (entry->actual_size % alloc_increment);
        if (avail_size < append_size) {
            *ppentry = (btree_journal_entry*)realloc((void*)entry,
                                                     sisl::round_up(entry->actual_size + append_size, alloc_increment));
        }
        return entry;
    }

    static void append_node(btree_journal_entry** ppentry, bt_journal_node_op node_op, bnodeid_t node_id, uint64_t gen,
                            sisl::blob key = {nullptr, 0}) {
        uint16_t append_size = sizeof(bt_journal_node_info) + key.size;
        btree_journal_entry* entry = realloc_if_needed(ppentry, append_size);
        ++entry->node_count;
...
private:
    btree_journal_entry(journal_op p) : op(p) {}
