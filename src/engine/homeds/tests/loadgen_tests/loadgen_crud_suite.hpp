...
struct BtreeLoadGen {
    ...
    bool increment_create() {
        std::unique_lock< std::mutex > lk(m_mtx);
        if ((uint64_t)(stored_keys + outstanding_create) >= p.NK) return false; // cant accomodate more
        outstanding_create++;
        return true;
    }
    ...
    bool try_verify_all() {
        if (C_NV > (int64_t)p.NK) { return true; }
    ...
    void regression(bool update_allowed, bool remove_allowed, bool range_update_allowed, bool range_query_allowed) {
        kvg->run_parallel([&]() {
            while (true) {
                ...
                if ((uint64_t)get_issued_ios() > p.NIO || get_elapsed_time(p.startTime) > p.NRT) break;
            }
        });
