...
public:
    ...
    static uint64_t get_nbits() {
        if (fixed_nbits) { return fixed_nbits; }
...
TEST_F(BitmapTest, ContiguousNbitsSeqFillTest) {
    uint64_t total_nbits = this->get_nbits();
    if (total_nbits < Mi) { total_nbits = Mi; }
...
TEST_F(BitmapTest, DeserializeTest) {
    uint64_t total_nbits = this->get_nbits();
    if (total_nbits < Ki) { total_nbits = Ki; }
...
int main(int argc, char* argv[]) {
    ...
    if (homeds::fixed_nbits) { assert(homeds::fixed_nbits >= MIN_LOWER_LIMIT); }
