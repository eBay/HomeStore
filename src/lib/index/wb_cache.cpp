...
void IndexWBCache::recover(sisl::byte_view sb) {
    ...
    auto persisted_root_was_committed = [&](uint32_t ordinal) {
        ...
        auto const it = bufs.find(blk_id{table->persisted_root_node_id()});
...
// Write the root-transition nodes of a root split ahead of the normal DAG flush. Takes the buffer list BY VALUE so the
// nodes -- and the memory the writes DMA out of -- stay alive in this coroutine frame across the fan-out await.
sisl::async::task< void > IndexWBCache::preflush_root_nodes(IndexCPContext* cp_ctx, IndexBufferPtrList preflush_bufs) {
    std::vector< sisl::async::task< iomgr::io_result > > preflush_tasks;
    preflush_tasks.reserve(preflush_bufs.size());
    for (auto const& buf : preflush_bufs) {
        LOGTRACEMOD(wbcache, "Preflushing root-transition node {}", buf->to_string());
        preflush_tasks.emplace_back(
            m_vdev->async_write(r_cast< const char* >(buf->raw_buffer()), m_node_size, buf->m_blkid, true));
    }

    for (auto const& result : co_await sisl::async::when_all(std::move(preflush_tasks))) {
        HS_REL_ASSERT(result, "Root-transition preflush failed with error={} ({})", result.error().value(),
...
sisl::async::task< bool > IndexWBCache::async_cp_flush(IndexCPContext* cp_ctx) {
    ...
    if (preflush_bufs.empty()) {
        ...
        // PhysicalDev batching is reactor-local, so submission must run on an I/O reactor with a drive channel.
        iomanager.run_on_forget(
            m_cp_flush_reactors.front(), [this, cp_ctx, preflush_bufs = std::move(preflush_bufs)]() mutable {
#ifdef _PRERELEASE
...
#endif
                // Unlike the folly future this replaces, task<> is lazy: when_all starts each child (queueing its
