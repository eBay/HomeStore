...
class DeviceManager {
    using vdev_info_found_cb_t = std::function< shared< VirtualDev >(const vdev_info&) >;
...
private:
    ...
    vdev_info_found_cb_t m_vdev_found_cb;

    // std::unique_ptr< ChunkManager > m_chunk_mgr;

public:
    DeviceManager(const std::vector< dev_info >& devs, vdev_info_found_cb_t vdev_found_cb);
    ...
    /// @return
    shared< VirtualDev > create_vdev(vdev_parameters&& vdev_param, vdev_info_found_cb_t&& vdev_found_cb);
