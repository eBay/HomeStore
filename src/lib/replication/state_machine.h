...
struct repl_req {
    ...
    RemoteBlkId remote_blkid;                    // Remote blkid for the value
    BlkId local_blkid;                           // Corresponding local blkid for the value
    void* user_ctx{nullptr};                     // User context passed with repldev::write, valid for leader only
    int64_t lsn{0};                              // Lsn for this replication req
    raft_buf_ptr_t journal_entry;                // Journal entry info
    std::atomic< uint32_t > num_blks_written{0}; // Total blks persisted in store
...
struct repl_journal_entry {
    ...
    uint32_t key_size;
    // Followed by user_header, then key, then blkid

public:
    uint32_t total_size() const { return sizeof(repl_journal_entry) + sizeof(BlkId) + user_header_size + key_size; }
};
...
using AsyncNotify = folly::Future< folly::Unit >;
using AsyncNotifier = folly::Promise< folly::Unit >;

ENUM(blkid_state_t, uint32_t,
     allocated,       // Starting state of all incoming blkids. Blks are allocated, but data is yet to be received
     write_scheduled, // Data has been received and write is scheduled
     completed        // Data has been written and available for mapping.
);

struct local_blkid_info {
    local_blkid_info(BlkId const& rb, BlkId const& lb) : m_rblkid{rb}, m_lblkid{lb} {}

    RemoteBlkId m_rblkid;
    BlkId m_lblkid;                                                 // a remote blk can map to local blkid
    std::atomic< blkid_state_t > m_state{blkid_state_t::allocated}; // state applies to all of the local blkids
};
...
private:
    // RemoteBlkId to local blkid mapping;
    folly::ConcurrentHashMap< RemoteBlkId, shared< repl_req > > m_blkid_map;
    folly::ConcurrentHashMap< int64_t, shared< repl_req > > m_lsn_req_map;
    ReplicaDevImpl* m_rd;
