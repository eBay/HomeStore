...
void FetchRPC::schedule_fetch_and_write(std::vector< repl_req_ptr_t > const& rreqs) {
    static thread_local std::vector< flatbuffers::Offset< RequestEntry > > entries;
    entries.clear();

    // Prepare the rpc request packet with all repl_reqs details
    flatbuffers::FlatBufferBuilder builder;
    for (const auto& rreq : rreqs) {
        entries.emplace_back(CreateRequestEntry(
            builder, rreq->rkey.lsn, rreq->rkey.term, rreq->rkey.dsn,
            CreateVector(builder, rreq->header.bytes, rreq->header.size),
            CreateVector(builder, rreq->key.bytes, rreq->key.size), rreq->remote_blkid.server_id,
            CreateVector(builder, rreq->remote_blkid.blkid.serialize(), rreq->remote_blkid.blkid.serialized_size())));
    }
    builder.Finish(CreateFetchDataRequest(builder, builder.CreateVector(entries)));

    io_blob_list_t pkts{io_blob{builder.GetBufferPointer(), builder.GetSize(), false}};
    m_sm->group_msg_service()
        ->data_service_request_bidirectional(nuraft_mesg::role_regex::LEADER, FETCH_DATA, pkts)
        .deferValue([this, pkts](sisl::io_blob const& incoming_buf) { on_fetch_data_response_received(incoming_buf); });
}
