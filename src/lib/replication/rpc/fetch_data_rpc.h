...
class FetchRPC {
public:
    FetchRPC::FetchRPC(ReplicaStateMachine* sm) : m_sm{sm} {}
    void schedule_fetch_and_write(std::vector< shared< repl_req > > const& reqs);

private:
    void on_fetch_data_request(sisl::io_blob const& incoming_buf, intrusive< sisl::GenericRpcData >& rpc_data);
    void on_fetch_data_request_completed(intrusive< sisl::GenericRpcData >& rpc_data);
    void on_fetch_data_response_received(sisl::io_blob const& incoming_buf);
    void validate_header(sisl::io_blob const& incoming_buf);

    template < typename T >
    std::pair< T*, sisl::io_blob_list_t > prepare_header(uint32_t addln_size) {
        ...
        rpc->group_id = m_sm->group_id;
        rpc->issuer_replica_id = m_sm->replica_id;
...
#pragma pack(1)
struct fetch_data_request : public data_channel_rpc_hdr {
    struct req_entry {
        uint32_t user_header_size;
        uint32_t key_size;
        RemoteBlkId remote_blkid;
        int64_t lsn;
        // Followed by header + key
    };

    struct req_iterator {
        fetch_data_request* req_;
    ...
    req_entry entries[0];
    // Followed by array of fetch entries

    // Followed by the usr_header + key for each entry
    static sisl::io_blob_list_t serialize(FetchRPC* f, repl_req_list_t const& rreqs) {
        auto [rpc, pkts] = f->prepare_header< fetch_data_request >(reqs.size() * sizeof(req_entry));
        rpc->num_blocks = reqs.size();

        for (uint32_t i{0}; i < rreqs.size(); ++i) {
            auto const& req = rreqs[i];
            entries[i].usr_header_size = req->header.size;
            entries[i].key_size = req->key.size;
            entries[i].lsn = req->lsn;
            entries[i].remote_blkid = req->remote_blkid;

            uint32_t buf_size = req->header.size + req->key.size;
            ...
            pkts.push_back(io_blob{buf, buf_size, false /* aligned*/});
        }
        return pkts;
    }
    ...
    static void free(FetchRPC* f, sisl::io_blob_list_t const& pkts) {
...
struct fetch_data_response : public data_channel_rpc_hdr {
    ...
    static sisl::io_blob_list_t serialize(FetchRPC* f, FetchRPCStatus status, fetch_result* result) {
        auto [rpc, bl] = f->prepare_header< fetch_data_reponse >(result->count() * sizeof(resp_entry));
        rpc->status = status;
