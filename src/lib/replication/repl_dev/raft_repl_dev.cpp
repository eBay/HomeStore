...
void RaftReplDev::update_truncation_boundary(repl_req_ptr_t rreq) {
    ...
    auto proposed_truncation_upper_limit = ctx->truncation_upper_limit;
    auto cur_truncation_upper_limit = m_truncation_upper_limit.load();
    // proposed_truncation_upper_limit might be less or equal to cur_truncation_upper_limit after Baseline Re-sync,
    ...
    if (proposed_truncation_upper_limit <= cur_truncation_upper_limit) {
        RD_LOGW(NO_TRACE_ID,
                "proposed_truncation_upper_limit {} is no larger than cur_truncation_upper_limit {}",
                proposed_truncation_upper_limit, cur_truncation_upper_limit);
        return;
    }

    while (cur_truncation_upper_limit < proposed_truncation_upper_limit &&
           !m_truncation_upper_limit.compare_exchange_weak(cur_truncation_upper_limit, proposed_truncation_upper_limit)) {}
    RD_LOGI(NO_TRACE_ID, "Raft repl update truncation_upper_limit to {}", proposed_truncation_upper_limit);
}
