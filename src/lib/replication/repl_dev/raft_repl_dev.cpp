...
RaftReplDev::RaftReplDev(RaftReplService& svc, superblk< raft_repl_dev_superblk >& rd_sb, bool load_existing) :
    ...
    m_msg_mgr->bind_data_service_request(PUSH_DATA, m_group_id, bind_this(RaftReplDev::on_push_data_received, 2));
...
void RaftReplDev::async_alloc_write(sisl::blob const& header, sisl::blob const& key, sisl::sg_list const& value,
    ...
    if (rreq->value.size) {
        rreq->rkey = repl_key{
            .server_id = server_id(), .term = raft_server()->get_term(), .dsn = 0}; // TODO: Generate correct DSN
...
void RaftReplDev::on_push_data_received(sisl::io_blob const& incoming_buf,
                                        intrusive< sisl::GenericRpcData >& rpc_data) {
    auto const fb_size = flatbuffers::ReadScalar< flatbuffers::uoffset_t >(incoming_buf.cbytes());
...
AsyncNotify RaftReplDev::notify_after_data_written(std::vector< repl_req_ptr_t >* rreqs) {
    ...
    rreqs->erase(std::remove_if(rreqs->begin(), rreqs->end(), [this, &futs](repl_req_ptr_t const& rreq) {
...
nuraft::ptr< nuraft::cluster_config > RaftReplDev::load_config() {
    ...
    if (js.empty()) {
        ...
        cluster_conf->get_servers().push_back(
            nuraft::cs_new< nuraft::srv_config >(m_raft_server_id, m_repl_svc.lookup_peer(m_my_repl_id)));
...
void RaftReplDev::report_committed(repl_req_ptr_t rreq) {
    ...
    RD_DBG_ASSERT_EQ(rreq->lsn, prev_lsn + 1, "Out of order commit of lsns, it is not expected in RaftReplDev");
