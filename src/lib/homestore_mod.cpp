...
bool HomeStore::start(const hs_input_params& input, hs_before_services_starting_cb_t svcs_starting_cb,
                      std::unique_ptr< IndexServiceCallbacks > index_cbs) {
    ...
    if (has_index_service()) { m_index_service = std::make_unique< IndexService >(std::move(index_cbs)); }
    if (has_replication_service()) {
        m_data_service = std::make_unique< BlkDataService >();
        m_repl_service = std::make_unique< ReplicationServiceImpl >(std::move(repl_cbs));
    }
...
void HomeStore::format_and_start(std::map< uint32_t, hs_format_params >&& format_opts) {
    ...
    for (const auto& [svc_type, fparams] : format_opts) {
        ...
        } else if ((svc_type & HS_SERVICE::REPLICATION) && has_replication_service()) {
            m_data_service->create_vdev(pct_to_size(fparams.size_pct, HSDevType::Data));
        }
    }

    if (!futs.empty()) {
        auto err = folly::collectAllUnsafe(futs).get();
        HS_REL_ASSERT(!err, "IO error during format of vdev, error={}", e.message());
    }
...
bool HomeStore::has_replication_service() const {
    return HomeStoreStaticConfig::instance().input.services.svcs & HS_SERVICE::REPLICATION;
}
...
void HomeStore::init_cache() {
    ...
    hs_config.engine.max_blks_in_blkentry =
        std::min(static_cast< uint32_t >(max_blks_per_blkid()), get_indx_mgr_page_size() / (4 * 2));
