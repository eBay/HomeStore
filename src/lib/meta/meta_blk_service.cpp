...
void MetaBlkService::create_vdev(uint64_t size) {
    ...
    hs()->device_mgr()->create_vdev(vdev_parameters{.vdev_name = "meta",
                                    ...
                                    bind_this(MetaBlkService::do_open_vdev, 1));
}

shared< VirtualDev > MetaBlkService::open_vdev(const vdev_info& vinfo) {
    do_open_vdev(vinfo);
...
shared< VirtualDev > MetaBlkService::do_open_vdev(const vdev_info& vinfo) {
