...
void HomeBlks::homeblks_sb_write() {
    if (m_sb_cookie == nullptr) {
        // add to MetaBlkMgr
        MetaBlkMgr::instance()->add_sub_sb("HOMEBLK", (void*)m_homeblks_sb.get(), sizeof(homeblks_sb), m_sb_cookie);
    } else {
        // update existing homeblks sb
        MetaBlkMgr::instance()->update_sub_sb("HOMEBLK", (void*)m_homeblks_sb.get(), sizeof(homeblks_sb), m_sb_cookie);
    }
}
