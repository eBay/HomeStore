...
void HomeBlks::create_volume(VolumePtr vol) {
    ...
    m_sobject->add_child(vol->sobject());
...
VolumePtr HomeBlks::create_volume(const vol_params& params) {
    ...
    m_sobject->add_child(new_vol->sobject());
...
sisl::status_response HomeBlks::get_status(const sisl::status_request& request) {
    ...
    auto hb{HomeBlks::safe_instance()};
    auto id = hb->get_index_blkstore()->sobject()->name();
