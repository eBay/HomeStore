...
Volume::Volume(const vol_params& params) :
    ...
    m_status_obj = m_hb->status_mgr()->create_object(params.vol_name, "volume",
...
Volume::Volume(meta_blk* mblk_cookie, sisl::byte_view sb_buf) :
    ...
    m_hb = HomeBlks::safe_instance();
    m_status_obj = m_hb->status_mgr()->create_object(sb->vol_name, "volume",
                                                     std::bind(&Volume::get_status, this, std::placeholders::_1));
}
...
void Volume::init() {
    ...
    if (m_sb_buf == nullptr) {
        ...
        m_status_obj->add_child(m_indx_mgr->status_object());
...
sisl::status_response Volume::get_status(const sisl::status_request& request) {
    ...
    auto active_indx_json = get_active_indx()->get_status(request).json;
    if (!active_indx_json.empty()) { response.json["index"] = active_indx_json; }

    response.json["name"] =  status_object()->name();
    response.json["type"] = status_object()->type();
