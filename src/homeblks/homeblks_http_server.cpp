...
void HomeBlksHttpServer::setup_routes() {
    ...
    try {
        ...
        http_server_ptr->setup_route(Http::Method::Get, "/api/v1/logstore/status",
                                     Routes::bind(&HomeBlksHttpServer::get_status_logstore, this));
        http_server_ptr->setup_route(Http::Method::Get, "/api/v1/logstore/:family/status",
                                     Routes::bind(&HomeBlksHttpServer::get_status_logstore_family, this));
        ...
        http_server_ptr->setup_route(Http::Method::Get, "/api/v1/logstore/:family/:logstore/status",
                                     Routes::bind(&HomeBlksHttpServer::get_status_logstore_by_id, this));
        http_server_ptr->setup_route(Http::Method::Get, "/api/v1/metablk",
                                     Routes::bind(&HomeBlksHttpServer::get_status_metablk, this));
...
void HomeBlksHttpServer::get_status_logstore(const Pistache::Rest::Request& request,
                                             Pistache::Http::ResponseWriter response) {
    std::string failure_resp{""};
    int verbosity_level{-1};
    if (!verify_and_get_verbosity(request, failure_resp, verbosity_level)) {
    ...
    // auto status_json = status_mgr->get_status(std::vector<std::string> {"LogStore"}, verbosity_level);
    const auto status_mgr = m_hb->status_mgr();
    auto status_json = HomeLogStoreMgrSI().get_status(verbosity_level);
    response.send(Pistache::Http::Code::Ok, status_json.dump());
}

void HomeBlksHttpServer::get_status_logstore_family(const Pistache::Rest::Request& request,
                                                    Pistache::Http::ResponseWriter response) {
    ...
    auto status_json = HomeLogStoreMgrSI().get_status_family(f_name, verbosity_level);
    response.send(Pistache::Http::Code::Ok, status_json.dump());
}

void HomeBlksHttpServer::get_status_logstore_by_id(const Pistache::Rest::Request& request,
                                                   Pistache::Http::ResponseWriter response) {
    auto f_name = request.param(":family").as< std::string >();
    auto l_id = request.param(":logstore").as< int >();
    std::string failure_resp{""};
    int verbosity_level{-1};
    if (!verify_and_get_verbosity(request, failure_resp, verbosity_level)) {
        response.send(Pistache::Http::Code::Bad_Request, failure_resp);
    ...
    auto status_json = HomeLogStoreMgrSI().get_status_logstore(f_name, l_id, verbosity_level);
    response.send(Pistache::Http::Code::Ok, status_json.dump());
}
...
void HomeBlksHttpServer::dump_logstore_family(const Pistache::Rest::Request& request,
    ...
    auto dlf = HomeLogStoreMgrSI().data_log_family();
    auto family = dlf;
    auto clf = HomeLogStoreMgrSI().ctrl_log_family();
    if (dlf->get_name() != f_name && clf->get_name() != f_name) {
        response.send(Pistache::Http::Code::Bad_Request, "inValid log family");
        return;
    } else if (clf->get_name() == f_name) {
        family = clf;
    }
...
// get_meta_blkstore
void HomeBlksHttpServer::get_status_metablk(const Pistache::Rest::Request& request,
                                            Pistache::Http::ResponseWriter response) {
    std::vector< std::string > modules;
    const auto modules_kv{request.query().get("module")};
    if (modules_kv) {
    ...
    const auto status_mgr = m_hb->status_mgr();
    auto status_json = status_mgr->get_status(std::vector< std::string >{"MetaBlkMgr"}, verbosity_level);
    response.send(Pistache::Http::Code::Ok, status_json.dump(2));
}
