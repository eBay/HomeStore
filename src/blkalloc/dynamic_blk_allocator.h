...
public:
    ...
    bool operator<(PageAllocSegment &other_seg) const {
        return (this->get_free_atoms() < other_seg.get_free_atoms());
    }

    void set_free_atoms(uint64_t freeAtoms) {
        m_free_atoms = freeAtoms;
    }

    uint64_t get_free_atoms() const {
        return m_free_atoms;
    }

    void set_total_atoms(uint64_t a) {
        m_total_atoms = a;
    }

    uint64_t get_total_atoms() const {
        return m_total_atoms;
    }
...
public:
    ...
    const BlkAllocConfig *get_config() const {
...
private:
    ...
    uint32_t size_to_nbits(uint32_t size) const {
        return size / get_config()->get_atom_size();
    }
...
public:
    ...
    uint32_t get_free_atoms_count() const {
        return (m_blob->m_nfree_atoms);
    }

    void set_free_atoms_count(uint32_t free_pieces) {
        m_blob->m_nfree_atoms = free_pieces;
    }

    uint32_t get_temperature() const {
        return (m_blob->m_temp);
    }
    ...
    uint64_t get_page_id() const {
        return m_blob->m_page_id;
    }

    inline void set_page_id(uint64_t pageId) {
        m_blob->m_page_id = pageId;
    }

    uint32_t get_max_contigous_free_atoms() const {
        return m_blob->m_max_contigous_free_atoms;
    }

    inline void set_max_contigous_free_atoms(uint32_t f) {
        m_blob->m_max_contigous_free_atoms = f;
    }
    ...
    int compare_range(const omds::btree::BtreeKey *s, bool start_incl, const omds::btree::BtreeKey *e,
        ...
        ret = is_in_range(this->get_temperature(), start->get_temperature(), start_incl,
                          end->get_temperature(), end_incl);
        if (ret != 0) {
            return ret;
        }
    ...
    virtual uint32_t get_blob_size() const {
        return (sizeof(DynamicPageAllocCacheEntry));
    }
    ...
    void print() {
        cout << "free_atom_count: " << get_free_atoms_count() << " temp: " << get_temperature() << " pageid: "
             << get_page_id();
    }
};
